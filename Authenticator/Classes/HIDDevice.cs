...
namespace Authenticator {
  public class HidDevice : IDisposable {
    ...
    [DllImport("kernel32.dll")]
    private static extern int WriteFile(SafeFileHandle hFile, ref byte lpBuffer, int nNumberOfBytesToWrite,
    ...
    public static List<HidDeviceEntry> GetAllDevices(Guid guid, int? vendorId = null, int? productId = null) {
      ...
      var devicesHandle =
        SetupDiGetClassDevs(ref guid, IntPtr.Zero, IntPtr.Zero, DIGCF_INTERFACEDEVICE | DIGCF_PRESENT);
      ...
      while (SetupDiEnumDeviceInterfaces(devicesHandle, IntPtr.Zero, ref guid, index, ref diData)) {
        ...
        if (SetupDiGetDeviceInterfaceDetail(devicesHandle, ref diData, ref diDetail, size, out size, IntPtr.Zero)) {
          ...
          var handle = CreateFile(diDetail.DevicePath, 4 /*GENERIC_WRITE*/, 3 /*FILE_SHARE_READ | FILE_SHARE_WRITE*/,
            IntPtr.Zero, 4 /*OPEN_EXISTING*/, 0, IntPtr.Zero);
    ...
    private void _Init(string devicePath, bool throwNotFoundError) {
      ...
      do {
        ...
        result = SetupDiEnumDeviceInterfaces(hDeviceInfo, IntPtr.Zero, ref guid, deviceCount,
          ref spDeviceInterfaceData);
        SetupDiGetDeviceInterfaceDetail(hDeviceInfo, ref spDeviceInterfaceData, IntPtr.Zero, 0, out var requiredSize,
          IntPtr.Zero);
        size = requiredSize;
        var diDetail = new SpDeviceInterfaceDetailData();
        diDetail.cbSize = (uint) (IntPtr.Size == 8 ? 8 : 5);
        ...
        if (this.devicePath == devicePath) {
          ...
          size = 0;
          requiredSize = 0;
          SetupDiGetDeviceInterfaceDetail(hDeviceInfo, ref spDeviceInterfaceData, IntPtr.Zero, size, out requiredSize,
            IntPtr.Zero);
          SetupDiGetDeviceInterfaceDetail(hDeviceInfo, ref spDeviceInterfaceData, IntPtr.Zero, size, out requiredSize,
            IntPtr.Zero);
          hidHandle = CreateFile(this.devicePath, (uint) FileAccess.ReadWrite, (uint) FileShare.ReadWrite, IntPtr.Zero,
            (uint) FileMode.Open, (uint) EFileAttributes.Overlapped, IntPtr.Zero);
