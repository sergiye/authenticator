    ...
    {    
        ...
        private bool VerifyAuthenticator(string privatekey)
        {
            ...
            if (match.Success)
            {
                ...
                {
                    MainForm.ErrorDialog(Owner, "Only time-based (TOTP) authenticators are supported when adding an Okta Verify Authenticator. Use the general \"Add Authenticator\" for counter-based (HOTP) authenticators.");
                    return false;
                }
